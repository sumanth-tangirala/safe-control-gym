...
#SBATCH --array=0-39
#SBATCH --output=logs/cp_mid_%A_%a.out
...
ROOT=${CP_SWEEP_OUT:-/scratch/$USER/cp_mid_levels}
: "${CP_DET_DIR:?set CP_DET_DIR to the directory holding eval_states.txt}"
mkdir -p "$ROOT"

# alpha beta, one config per line, index = config number.
CONFIGS=(
  "1.021   3.780"     # med candidate at ratio 0.27, between 0.1270 and 0.1495
  "1.080   4.000"     # ratio 0.27
  "3.800   1.000"     # med candidate at ratio 3.80, between 0.1280 and 0.1700
  "4.180   1.100"     # toward the mid ceiling
  "4.940   1.300"     # peak probe: 4.560/1.200 is the best seen at 0.1700
)
NSHARDS=8
