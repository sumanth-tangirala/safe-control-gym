...
#SBATCH --array=0-87
#SBATCH --output=logs/cp_isweep_%A_%a.out
...
ROOT=${CP_SWEEP_OUT:-/scratch/$USER/cp_interior_sweep}
: "${CP_DET_DIR:?set CP_DET_DIR to the directory holding eval_states.txt}"
mkdir -p "$ROOT"

# alpha beta, one config per line, index = config number.
CONFIGS=(
  "0.0     0.0"      # gate: must reproduce the deterministic labels, K=1
  "0.027   0.10"     # ratio 0.27
  "0.081   0.30"
  "0.243   0.90"
  "0.729   2.70"
  "2.160   8.00"
  "0.300   0.30"     # ratio 1.00
  "0.900   0.90"
  "2.700   2.70"
  "1.140   0.30"     # ratio 3.80 (the variance-matched character)
  "3.420   0.90"
)
NSHARDS=8
