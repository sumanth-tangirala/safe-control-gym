...
#SBATCH --array=0-63
#SBATCH --output=logs/cp_iref_%A_%a.out
...
ROOT=${CP_SWEEP_OUT:-/scratch/$USER/cp_interior_refine}
: "${CP_DET_DIR:?set CP_DET_DIR to the directory holding eval_states.txt}"
mkdir -p "$ROOT"

# alpha beta, one config per line, index = config number.
CONFIGS=(
  "0.405   1.50"     # ratio 0.27, filling in between 0.90 and 2.70
  "0.540   2.00"
  "0.945   3.50"     # bracketing the interior peak seen near beta 2.7
  "1.215   4.50"
  "1.620   6.00"     # between the peak and the collapse at beta 8
  "0.000   2.70"     # pure signal: no floor at all, noise vanishes at the goal
  "2.000   0.00"     # pure floor: constant noise, no signal dependence
  "5.000   0.00"     # constant, stronger -- is a wide band reachable at all?
)
NSHARDS=8
