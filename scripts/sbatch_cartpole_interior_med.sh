...
#SBATCH --array=0-23
#SBATCH --output=logs/cp_imed_%A_%a.out
...
ROOT=${CP_SWEEP_OUT:-/scratch/$USER/cp_interior_med}
: "${CP_DET_DIR:?set CP_DET_DIR to the directory holding eval_states.txt}"
mkdir -p "$ROOT"

# alpha beta, one config per line, index = config number.
CONFIGS=(
  "1.900   0.500"     # the gap between 0.450 (0.1115) and 0.635 (0.1905),
  "2.143   0.564"     # where interior rises steeply -- med lives in here
  "2.280   0.600"
)
NSHARDS=8
